...
SCSFExport scsf_NetVolumeCalculation(SCStudyInterfaceRef sc)
{
    ...
    SCInputRef Bullishcolor_opacity=sc.Input[14];
    ...
    if (sc.SetDefaults)
    {
        ...
        sc.Subgraph[2].PrimaryColor = RGB(120, 123, 13); // Gray for neutral
        ...
        Bullishcolor_opacity.Name="Bullish Opacity";
        Bullishcolor_opacity.SetFloat(90);
        Bullishcolor_opacity.SetFloatLimits(1,INT_MAX);
   ...
   if (downn)
    ...
    if (cnv_tb >= 0)
    {
        if (extra_extreme_sell)
            ...
            lineColor=frontier_buy_color;
        else if (sc.Close[sc.Index]<sc.Open[sc.Index])
            lineColor=sea_zone_downbar_color;
    }
    else
    {
        if (extra_extreme_buy)
            ...
            lineColor = frontier_sell_sweep_color;
             ...
             lineColor=frontier_sell_color;
        else if (sc.Close[sc.Index]>sc.Open[sc.Index])
        lineColor=sea_zone_upbar_color;
    }
